...
As of 2026-05-07 21:15 UTC, `npx --yes clawhub@0.12.2 package inspect @xquik/tweetclaw --json` reports ClawHub latest as `1.6.25` with `artifact.kind: "npm-pack"`, owner `kriptoburak`, `code-plugin` family, OpenClaw compatibility `2026.5.4`, package `scanStatus: "pending"`, and verification `scanStatus: "pending"` after explicit rescan request `sd7ds0wqjw6h641y9r835ehreh869fqq` was accepted. Version-specific inspect with `--version 1.6.25` reports a clean static scan with no static findings on engine `v2.4.22`. Treat the pending top-level package and verification statuses as ClawHub scan lag unless a later inspect reports suspicious findings.
