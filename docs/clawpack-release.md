...
As of 2026-05-06, `npx --yes clawhub@0.12.2 package inspect @xquik/tweetclaw --json` reports ClawHub latest as `1.6.19` with `artifact.kind: "npm-pack"`, owner `kriptoburak`, `code-plugin` family, OpenClaw compatibility `2026.5.4`, package `scanStatus: "clean"`, and verification `scanStatus: "clean"` after explicit rescan request `sd7fzwy0r5r6ajjbpyg5nvz805866cmh` completed clean. Version-specific inspect reports the `1.6.19` static scan clean with no findings on engine `v2.4.22`. The legacy ZIP warning remains resolved for the current package listing.
