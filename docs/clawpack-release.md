...
As of 2026-05-07 15:22 UTC, `npx --yes clawhub@0.12.2 package inspect @xquik/tweetclaw --json` reports ClawHub latest as `1.6.22` with `artifact.kind: "npm-pack"`, owner `kriptoburak`, `code-plugin` family, OpenClaw compatibility `2026.5.4`, package `scanStatus: "clean"`, and verification `scanStatus: "clean"` after explicit rescan request `sd7bfyr8ar39xaqpwfw5xj9v29868w7a` completed clean. Version-specific inspect reports the `1.6.22` static scan clean with no findings on engine `v2.4.22`. The legacy ZIP warning remains resolved for the current package listing.
