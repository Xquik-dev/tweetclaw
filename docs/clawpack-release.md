...
As of 2026-05-08 02:38 UTC, npm latest is `@xquik/tweetclaw@1.6.27`, published from commit `88b7879ed62c1a47ed33833afabe4654d9d86d9c`. GitHub release `v1.6.27` is published at https://github.com/Xquik-dev/tweetclaw/releases/tag/v1.6.27 for that source commit. ClawHub latest remains `1.6.26` with `artifact.kind: "npm-pack"`, owner `kriptoburak`, `code-plugin` family, package `scanStatus: "clean"`, and verification `scanStatus: "clean"` after explicit rescan request `sd794yd3eeb4av48t4tta8cyt186by6p` completed clean. Version-specific inspect with `--version 1.6.26` reports a clean static scan with no static findings on engine `v2.4.22`.
