...
As of 2026-05-08 01:01 UTC, `npx --yes clawhub@0.12.2 package inspect @xquik/tweetclaw --json` reports ClawHub latest as `1.6.26` with `artifact.kind: "npm-pack"`, owner `kriptoburak`, `code-plugin` family, OpenClaw compatibility `2026.5.4`, package `scanStatus: "pending"`, and verification `scanStatus: "pending"` after explicit rescan request `sd794yd3eeb4av48t4tta8cyt186by6p` was accepted. Version-specific inspect with `--version 1.6.26` reports a clean static scan with no static findings on engine `v2.4.22`.
