...
As of 2026-05-07 20:09 UTC, `npx --yes clawhub@0.12.2 package inspect @xquik/tweetclaw --json` reports ClawHub latest as `1.6.24` with `artifact.kind: "npm-pack"`, owner `kriptoburak`, `code-plugin` family, OpenClaw compatibility `2026.5.4`, package `scanStatus: "clean"`, and verification `scanStatus: "clean"` after explicit rescan request `sd78ne5291p1jgkkmvn39898zx869fxp` completed clean. Version-specific inspect with `--version 1.6.24` reports a clean static scan with no static findings on engine `v2.4.22`. The legacy ZIP warning remains resolved for the current package listing.
