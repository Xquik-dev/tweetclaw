...
- GitHub repo metadata as of 2026-05-09 14:03 UTC: public, MIT, default branch `master`, 37 stars, 3 forks, description mentions posting tweets, replies, likes, retweets, follows, DMs, OpenClaw, structured Xquik endpoints, and 99 agent-callable endpoints; topics include OpenClaw, TweetClaw, Xquik, X/Twitter, MCP, pay-per-use, skills, automation, ClawHub, Context7, social media, and data extraction.
- Open GitHub issues and PRs in `Xquik-dev/tweetclaw`: none open as of 2026-05-09 14:03 UTC. Closed issue `#1` remains the only historical issue.
...
- Current observed benchmark on 2026-05-09 14:03 UTC: the visible public benchmark page showed score `78.7`, Trust Score `7.5`, 180 snippets, and `Update: 48 minutes ago`. At 2026-05-09 13:51 UTC the same public page still showed `84.1`, so keep treating this swing as Context7 cache/version drift rather than a TweetClaw source change.
...
2026-05-09 14:03 UTC:

- Local repo clean at `742de5e` before edits; no untracked non-ignored files.
...
- Context7 public benchmark now visibly shows `78.7`, Trust Score `7.5`, 180 snippets, and `Update: 48 minutes ago`, after the same page showed `84.1` earlier in the day. Keep treating the benchmark as cache/version-drifted.
- Hygiene scan: `docs/clawpack-release.md`, `package.json`, `openclaw.plugin.json`, `server.json`, and `skills/tweetclaw/SKILL.md` were rechecked. The tracked-file secret-like path scan again found no matches. The automation file already shows `FREQ=MINUTELY;INTERVAL=5`, so no cadence change was needed.
- External placement action: opened https://github.com/GetBindu/awesome-claude-code-and-skills/pull/24 after reading README, Apache-2.0 license, repo metadata, recent PR history, prior closed x-twitter-scraper PR 7, and duplicate terms. The PR adds TweetClaw to `Domain-Specific Skills` with concrete wording around search tweets, search tweet replies, post tweets, follower export, media workflows, monitors, webhooks, and giveaway draws.
- External validation: `git diff --check` passed in the fork before commit, the PR body was read back after creation, and no NHS badge change was made because this target has no badge block where the xquik.com readiness badge would help readers evaluate one entry.
