...
- GitHub repo metadata as of 2026-05-09 11:05 UTC: public, MIT, default branch `master`, 37 stars, 3 forks, description mentions posting tweets, replies, likes, retweets, follows, DMs, OpenClaw, structured Xquik endpoints, and 99 agent-callable endpoints; topics include OpenClaw, TweetClaw, Xquik, X/Twitter, MCP, pay-per-use, skills, automation, ClawHub, Context7, social media, and data extraction.
- Open GitHub issues and PRs in `Xquik-dev/tweetclaw`: none as of 2026-05-09 11:05 UTC.
...
2026-05-09 11:05 UTC:

- Local repo clean at `5411e62` before edits; no untracked non-ignored files.
...
- Context7 static HTML plus embedded page data now show benchmark score `83.4`, Trust Score `7.5`, 180 snippets, 8 pages, 0 parse failures, last update `2026-05-09T10:56:13.547Z`, and parsed source SHA `5411e6229309d29eef5db35cbf550c89d40a8a36`. Old finalized versions still appear and generated `llms.txt` still contains stale 32-endpoint/media-download snippets, so the next useful action remains Context7 Versions-tab cleanup or documented owner-side cache removal.
- Context7 public `GET /api/v2/context?libraryId=/xquik-dev/tweetclaw` currently returns `validation_error`, so the public API shape still should not be used as evidence until official docs and the live endpoint agree again.
- Security and registry checks: Dependabot open alerts are 0, and `npm audit --audit-level=low` reports 0 vulnerabilities.
- External placement action: opened https://github.com/MobileFirstLLC/social-media-hacker-list/pull/131 after reading README, code of conduct, PR checklist, repo metadata, issue state, PR history, and duplicate search results. The PR adds TweetClaw to the X section with concrete wording for search tweets, post tweets and replies, follower export, monitors, webhooks, direct messages, and giveaway draws.
