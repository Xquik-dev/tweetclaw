...
- GitHub repo metadata as of 2026-05-09 03:26 UTC: public, MIT, default branch `master`, 37 stars, 3 forks, description mentions posting tweets, replies, likes, retweets, follows, DMs, OpenClaw, structured Xquik endpoints, and 99 agent-callable endpoints; topics include OpenClaw, TweetClaw, Xquik, X/Twitter, MCP, pay-per-use, skills, automation, ClawHub, Context7, social media, and data extraction.
- Open GitHub issues and PRs in `Xquik-dev/tweetclaw`: none as of 2026-05-09 03:26 UTC.
...
- Current observed benchmark on 2026-05-09 03:26 UTC: static HTML page data showed benchmark score `82.6`, Trust Score `7.5`, 180 snippets, 8 pages, 0 parse failures, last update `2026-05-09T00:51:33.456Z`, and parsed source SHA `0204b95dbf8a270859f093aae0e2d52d3ed3e669`. The visible/static snippet count still drifts between rendered page elements and embedded data; treat this as Context7 presentation or cache drift.
...
- OpenClaw npm latest and beta are both `2026.5.7` as of 2026-05-09 01:52 UTC.
...
- Local repo clean at `74a4717` before edits; no untracked non-ignored files.
...
- External placement action: amended https://github.com/sickn33/antigravity-awesome-skills/pull/582 on branch `codex/refresh-x-twitter-scraper-skill` at commit `c949083e0f3f8b99c8c1620609a6e929652e6350` to add the TweetClaw OpenClaw plugin install path and search-intent use cases to the existing Xquik `x-twitter-scraper` skill.
- External validation: read `sickn33/antigravity-awesome-skills` README, CONTRIBUTING, code of conduct, security policy, MIT license, PR template, quality bar, security guardrails, open PRs/issues, duplicate terms, and existing PR 582. Ran `npm run validate`, `npm run security:docs`, `git diff --check`, description-length check, and changed-diff secret/internal wording scan; pushed the branch and updated the PR body with real newlines.
- Security follow-up: GitHub push reported 3 Dependabot alerts, and the Dependabot API plus `npm audit` showed all mapped to transitive `hono <4.12.18` through the lockfile's OpenClaw/MCP SDK path. `npm audit fix` updated the lockfile to `hono 4.12.18`; `npm audit --json` now reports 0 vulnerabilities, and a follow-up Dependabot API check returned 0 open alerts.
- TweetClaw validation: `npm run check:all`, `npm pack --dry-run --json`, and `node scripts/clawpack.mjs dry-run` passed after the lockfile security update.
