...
- Version-specific ClawHub inspect for `1.6.20` reported `staticScan.status: "clean"` with no findings on engine `v2.4.22`. Explicit package rescan request `sd7e9jw78zcqgtgr10gs71ps0n866f4k` was accepted; top-level package inspect still reported package `scanStatus: "pending"` and verification `scanStatus: "pending"` at 2026-05-06 23:39 UTC.
