...
- OpenClaw npm checked as of 2026-06-17 04:29 UTC: latest
...
- ClawHub route `clawhub.ai/plugins/@xquik/tweetclaw` returned HTTP 200
  on 2026-06-17 04:29 UTC, but latest ClawHub page remains `1.6.26` and
...
- 2026-06-17 04:29 UTC: model/effort selectors unavailable. Crawler returned 0
  opportunities. TweetClaw and Xquik issue checks were clean. Readback of 100
  open outreach PRs found no repairable maintainer request, CI failure,
  conflict, or review finding; bot notes and target-owned deploy authorization
  stayed non-actionable.
- Fresh PR: https://github.com/thevrus/pubcraft/pull/1 adds X/Twitter
  source-evidence packets to Pubcraft, frames TweetClaw/OpenClaw as optional
  evidence collection only, and rebuilds the tracked package zip. Readback:
  open, mergeable, ready, no checks configured.
- Checks: target build, zip test, frontmatter check, focused target links,
  public hygiene, OpenClaw docs/packages, public URL probes, package dry run,
  build, and TweetClaw `check:all` passed. Target `SOURCES.md` had pre-existing
  403s on doi.org and mdpi.com; npm audit still reports nested OpenClaw
  advisories.
