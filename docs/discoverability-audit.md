...
- GitHub repo metadata as of 2026-05-09 11:22 UTC: public, MIT, default branch `master`, 37 stars, 3 forks, description mentions posting tweets, replies, likes, retweets, follows, DMs, OpenClaw, structured Xquik endpoints, and 99 agent-callable endpoints; topics include OpenClaw, TweetClaw, Xquik, X/Twitter, MCP, pay-per-use, skills, automation, ClawHub, Context7, social media, and data extraction.
- Open GitHub issues and PRs in `Xquik-dev/tweetclaw`: none as of 2026-05-09 11:22 UTC.
...
2026-05-09 11:22 UTC:

- Local repo clean at `2318acd` before edits; no untracked non-ignored files.
...
- Context7 benchmark state is unchanged from the 11:05 UTC check: score `83.4`, Trust Score `7.5`, 180 snippets, 8 pages, 0 parse failures, last update `2026-05-09T10:56:13.547Z`, parsed source SHA `5411e6229309d29eef5db35cbf550c89d40a8a36`, old finalized versions still present, and generated `llms.txt` still containing stale 32-endpoint/media-download snippets.
...
- External placement action: opened https://github.com/jivoi/awesome-osint/pull/930 after reading README, CONTRIBUTING, repo metadata, issue state, PR history, existing merged Xquik PR 812, and duplicate search results. The PR adds TweetClaw to the Twitter section with OSINT-oriented wording for tweet search, user lookup, follower export, reply and quote extraction, account monitoring, and structured X/Twitter data collection.
- External validation: `git diff --check` passed in the fork, and the PR body was read back from GitHub after creation. The NHS badge was not added because the target badge area is for the directory itself, and an xquik.com readiness badge would not help maintainers evaluate that list entry.
