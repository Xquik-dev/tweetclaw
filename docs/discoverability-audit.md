...
- The legacy `tweetclaw` skill was restored and unhidden on 2026-05-06 after the publisher decided to keep a TweetClaw skill entry. It still belongs to owner `xquik`, latest version `1.1.8`, and ClawHub inspection reports a `suspicious` skill verdict against that old version.
- Publishing a fresh `tweetclaw` skill version from this repo still fails while logged in as `kriptoburak` because the slug is owned by the separate `xquik` ClawHub account. Updating that skill requires logging the ClawHub CLI in as owner `xquik`, transferring the skill, or using an owner-approved ClawHub moderation workflow.
