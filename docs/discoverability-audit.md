...
- ClawHub package rescan request `sd70hgg2acj0zps78m1edrk1vx864vge` was accepted for `1.6.12`; ClawHub inspection still reports `scanStatus: pending` as of 2026-05-06 23:17 UTC.
...
- The legacy `tweetclaw` skill was hidden and then soft-deleted through the ClawHub CLI on 2026-05-06 with the reason that it is superseded by the `@xquik/tweetclaw` OpenClaw plugin package and ClawPack release. `clawhub inspect tweetclaw --json` now returns `Skill not found`.
- The direct legacy security URL may still render stored scan output even though normal skill inspection returns not found. Future runs should check both the CLI lookup and the public route; if the direct security route remains visible, use the `xquik` owner account or ClawHub moderation support to remove or redirect the stale route.
...
- Do not recreate the legacy `tweetclaw` skill slug unless logged in as the `xquik` owner account and publishing an intentional redirect-style skill that points users to `openclaw plugins install @xquik/tweetclaw`.
