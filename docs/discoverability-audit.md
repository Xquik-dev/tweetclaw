...
- GitHub repo metadata as of 2026-05-08 17:56 UTC: public, MIT, default branch `master`, 37 stars, 3 forks, description mentions posting tweets, replies, likes, retweets, follows, DMs, OpenClaw, structured Xquik endpoints, and 99 agent-callable endpoints; topics include OpenClaw, TweetClaw, Xquik, X/Twitter, MCP, pay-per-use, skills, automation, ClawHub, Context7, social media, and data extraction.
- Open GitHub issues and PRs in `Xquik-dev/tweetclaw`: none as of 2026-05-08 17:56 UTC.
...
- Current observed benchmark on 2026-05-08 17:56 UTC: static benchmark data showed score `85.4`, 180 snippets, 8 pages, 0 parse failures, and last update `2026-05-08T12:23:05.519Z`. The rendered public page also showed Benchmark `85.4`, 202 snippets, Trust Score `7.5`, and "Update: 5 hours ago"; treat the snippet-count mismatch as Context7 presentation or cache drift.
...
- OpenClaw npm latest and beta are both `2026.5.7` as of 2026-05-08 17:56 UTC.
...
- Xquik public `llms.txt` and billing page say MPP covers 31 X-API read-only endpoints as of 2026-05-08 17:56 UTC. The billing page says media downloads require API key or session-cookie authentication because they create gallery links, although search snippets may still lag. Local Xquik source remains source of truth: `lib/mpp/pricing.test.ts` asserts 31 eligible routes and `POST /api/v1/x/media/download` is not MPP eligible; TweetClaw 1.6.25 clarifies that user media MPP means timeline reads, not media file download.
...
2026-05-08 17:56 UTC:

- Local repo clean at `0e1942b` before edits; no open TweetClaw repo issues or PRs.
...
- Context7 benchmark remains score `85.4`, with static data showing 180 snippets, 8 pages, 0 parse failures, and last update `2026-05-08T12:23:05.519Z`. The rendered page showed 202 snippets and Trust Score `7.5`; the old finalized versions `v1.6.12` through `v1.6.15` still appear and stale 32-endpoint/media-download snippets remain, so the next useful action is Context7 Versions-tab cleanup or documented owner-side cache removal.
...
- External placement action: opened https://github.com/littleben/awesomeAgentskills/pull/8 adding a TweetClaw skill directory plus English and Chinese README entries after confirming the contribution rules, MIT license, active repo state, open and closed PRs/issues, and no duplicate TweetClaw/Xquik references. The skill uses concrete search-intent wording for search tweets, post tweets, post tweet replies, follower export, media upload/download, tweet monitoring, webhooks, giveaway draws, OpenClaw, Xquik, and MPP. Verified PR Markdown headings, checklists, links, and no literal `\n` sequences.
- No-action placement audits this run: `shaypal5/awesome-twitter-data` already has Xquik PR 10 open; `scienceaix/agentskills` is currently a high-star framework and paper list where a TweetClaw row would be off-scope; `futantan/agent-skills.md` already has open Xquik issue 16 and no detected license file, so no duplicate PR was opened.
