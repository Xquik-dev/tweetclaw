...
- GitHub repo metadata as of 2026-05-09 13:16 UTC: public, MIT, default branch `master`, 37 stars, 3 forks, description mentions posting tweets, replies, likes, retweets, follows, DMs, OpenClaw, structured Xquik endpoints, and 99 agent-callable endpoints; topics include OpenClaw, TweetClaw, Xquik, X/Twitter, MCP, pay-per-use, skills, automation, ClawHub, Context7, social media, and data extraction.
- Open GitHub issues and PRs in `Xquik-dev/tweetclaw`: none open as of 2026-05-09 13:16 UTC. Closed issue `#1` remains the only historical issue.
...
- Current observed benchmark on 2026-05-09 13:51 UTC: the visible public benchmark page still showed score `84.1`, Trust Score `7.5`, 180 snippets, and recent update text, but the same page's embedded library JSON still exposed the older finalized snapshot with score `78.7`, 8 pages, 0 parse failures, last update `2026-05-09T12:46:02.093Z`, and parsed source SHA `987c074442482c9fbd1fcb1feb69247ff368fd39`. Treat this visible-vs-embedded mismatch as more evidence of Context7 cache/version drift.
...
2026-05-09 13:51 UTC:

- Local repo clean at `d4fb804` before edits; no untracked non-ignored files.
- npm latest remains `@xquik/tweetclaw@1.6.27` with gitHead `88b7879ed62c1a47ed33833afabe4654d9d86d9c`; OpenClaw latest remains `2026.5.7` and beta is now `2026.5.9-beta.1`; GitHub release `v2026.5.7` remains the latest stable tag.
- GitHub repo metadata remains public, MIT, 37 stars, 3 forks, and no open TweetClaw issues or PRs.
- Context7 public benchmark still shows a split view: visible benchmark `84.1` with Trust Score `7.5` and 180 snippets, but embedded page data still exposes the older finalized `78.7` snapshot with source SHA `987c074442482c9fbd1fcb1feb69247ff368fd39`. Keep treating Context7 cache/version drift as the blocker.
- Hygiene scan: `docs/clawpack-release.md`, `openclaw.plugin.json`, `server.json`, `skills/tweetclaw/SKILL.md`, `context7.json`, and `.gitignore` were rechecked. The tracked-file secret-like path scan again found no matches.
- External placement action: opened https://github.com/BehiSecc/awesome-claude-skills/pull/288 after reading README, the inline contribution section, repo metadata, open and closed PRs/issues, duplicate terms, and the previously merged x-twitter-scraper PR 81. The PR adds TweetClaw to `Utility & Automation` as the automation-oriented companion to the existing Xquik data-extraction entry, with concrete wording around search tweets, search tweet replies, post tweets, follower export, media workflows, monitors, webhooks, and giveaway draws.
