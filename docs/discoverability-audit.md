...
- GitHub repo metadata as of 2026-05-08 19:04 UTC: public, MIT, default branch `master`, 37 stars, 3 forks, description mentions posting tweets, replies, likes, retweets, follows, DMs, OpenClaw, structured Xquik endpoints, and 99 agent-callable endpoints; topics include OpenClaw, TweetClaw, Xquik, X/Twitter, MCP, pay-per-use, skills, automation, ClawHub, Context7, social media, and data extraction.
- Open GitHub issues and PRs in `Xquik-dev/tweetclaw`: none as of 2026-05-08 19:04 UTC.
...
- OpenClaw npm latest and beta are both `2026.5.7` as of 2026-05-08 19:04 UTC.
...
- Xquik public `llms.txt` and billing page say MPP covers 31 X-API read-only endpoints as of 2026-05-08 19:04 UTC. The billing page says media downloads require API key or session-cookie authentication because they create gallery links, although search snippets may still lag. Local Xquik source remains source of truth: `lib/mpp/pricing.test.ts` asserts 31 eligible routes and `POST /api/v1/x/media/download` is not MPP eligible; TweetClaw 1.6.25 clarifies that user media MPP means timeline reads, not media file download.
...
- Local repo clean at `73a57eb` before edits; no open TweetClaw repo issues or PRs.
...
- External placement action: opened https://github.com/LHL3341/awesome-claws/pull/12 adding TweetClaw to the bilingual Content & Creator Workflows section after confirming the MIT license, repository criteria, open issue, open and closed PRs, existing list format, and no duplicate TweetClaw/Xquik references. The entry uses concrete discovery wording for X/Twitter automation, search tweets and replies, post tweets, follower export, media upload/download, account monitoring, webhooks, giveaway draws, OpenClaw, and Xquik. Verified PR Markdown headings, bullets, links, and no literal `\n` sequences.
- No-action placement audits this run: `openclawdir.com` already lists TweetClaw; `awesome-claw/awesome-claw` is smaller, has an "Other" license classification, and was not chosen over the active MIT `LHL3341/awesome-claws` directory; no repository was resolved for the checked `allclaw` and `openclawdir` GitHub owner guesses.
